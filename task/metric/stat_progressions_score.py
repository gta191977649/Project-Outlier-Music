...
def evaluate_completeness_score(window_size, song):
    ...
    for pat in result:
        ...
        scores.append(cadence+transition)
...
if __name__ == '__main__':
    ...
    for idx,song in enumerate(song_collections):
        window, score = evaluate_window_size(window_sizes, song)
        score_data["id"].append(song.id)
        score_data["optimal_window"].append(window)
        score_data["score"].append(score)
        print(f"{id},{window},{score},REMAIN: {len(song_collections)-idx}")

    pd.DataFrame(score_data).to_csv("./weigth_average_score.csv",index=False)